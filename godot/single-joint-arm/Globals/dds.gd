...
class DDSVariable:
	...
	func set_value(t : int, value, _len : int):
		if t != self.type:
			self.type = t
			self.prepare_packet()
		match self.type:
			...
			DDS_TYPE_BLOB:
				_len = value.size()
				self.packet.encode_u32(self.value_offset, _len)
				self.packet.resize(self.value_offset + 4 + _len)
				for i in range(_len):
					self.packet.encode_u8(self.value_offset + 4 + i,value[i])
...
func publish_from_remote(peer : PacketPeerUDP, packet : PackedByteArray):
	...
	self.subscribed_vars[name] = value
...
func publish(name : String, type : int, value, _len : int = 0):
	...
	#print(item)
	item.set_value(type, value, _len)
	item.publish()

func subscribe(name : String):
	self.subscribed_vars[name] = null
